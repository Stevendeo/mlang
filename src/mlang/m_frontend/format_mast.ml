...
let format_instruction fmt i =
  Com.Com_format.format_instruction format_variable Pp.string fmt i
...
let format_input_variable fmt (v : input_variable) =
  Format.fprintf fmt "%a %s %a %a %a : %s%a;" format_variable_name
    ...
    (Pp.option (Pp.unmark Com.Com_format.format_value_typ))
    v.input_typ
...
let format_atom = Com.Com_format.format_atom format_variable
