...
let format_expr_var ~(env : Types.env) (dgfip_flags : Dgfip_options.flags) fmt
    (ev : expr_var) =
  match ev with
  ...
  | M (m_sp_opt, var, df) ->
      ...
      Format.fprintf fmt "%s"
        (generate_variable ~env ~trace_flag:dgfip_flags.flg_trace ~def_flag
           m_sp_opt var)

let rec format_dexpr ~(env : Types.env) (dgfip_flags : Dgfip_options.flags) fmt
    (de : expr) =
  let format_dexpr = format_dexpr ~env dgfip_flags in
  match de with
  ...
  | Dvar evar -> format_expr_var ~env dgfip_flags fmt evar
  | Dand l ->
  ...
  | Dfun (funname, des) ->
      Format.fprintf fmt "@[<hov 2>%s(%a@])" funname
        (Format.pp_print_list
           ...
           format_dexpr)
        des
  | Dvarinfo v -> format_varinfo ~env dgfip_flags fmt v
...
and format_varinfo ~env dgfip_flags fmt : varinfo_access -> unit = function
  | VIvar v -> Format.fprintf fmt "%s" (VID.gen_info_ptr v)
  | VItab (v, def, value) ->
      Format.fprintf fmt "@[lis_tabaccess_varinfo(irdata, %d, %a, %a)@]"
        (Com.Var.loc_tab_idx v)
        (format_dexpr ~env dgfip_flags)
        def
        (format_dexpr ~env dgfip_flags)
        value
...
let format_local_vars_defs ~env (dgfip_flags : Dgfip_options.flags) fmt
    (lv : local_vars) =
...
let format_assign ~env (dgfip_flags : Dgfip_options.flags) (var : string) fmt
    ((e, _kind, lv) : t) =
  Format.fprintf fmt "%a@;@[<hov 2>%s =@ %a;@]"
    (format_local_vars_defs ~env dgfip_flags)
    lv var
    (format_dexpr ~env dgfip_flags)
    e

let format_set_vars ~env (dgfip_flags : Dgfip_options.flags) fmt
  ...
  List.iter
    (fun ((_kd, vn, expr) : dflag * string * t) ->
      format_assign ~env dgfip_flags vn fmt expr)
    set_vars
...
let write_atomic_decoupled_expr ~env dgfip_flags oc res_def res_val
  ...
  if is_always_true def then
    pr "@;@[<v 2>{%a%a%a%a@]@;}" format_local_declarations locals
      (format_set_vars ~env dgfip_flags)
      set
      (format_assign ~env dgfip_flags res_def)
      def
      (format_assign ~env dgfip_flags res_val)
      value
  else
...
let rec write_decoupled_expr ~env dgfip_flags oc =
  let pr form = Format.fprintf oc form in
  fun res_def res_val -> function
    | `Atom a ->
        write_atomic_decoupled_expr ~env dgfip_flags oc res_def res_val a
    | `Cond (c, t, e) ->
        let d, v = fresh_cond_vars () in
        pr "@;{@[<v 2>";
        pr "@;int %s;" d;
        pr "@;double %s;" v;
        write_decoupled_expr ~env dgfip_flags oc d v c;
        pr "@;if(%s == 0) {%s = 0; %s = 0.0;}" d res_def res_val;
        pr "@;else if (EQ_E(%s,0.0)) {@;@[<v 2>" v;
        write_decoupled_expr ~env dgfip_flags oc res_def res_val e;
        pr "@;}@] else {@;@[<v 2>";
        write_decoupled_expr ~env dgfip_flags oc res_def res_val t;
        pr "@;}@]";
        pr "@;}@]"
    | `Let (vardef, varval, body, followup) ->
        pr "@;{@[<v 2>";
        pr "@;int %s;" vardef;
        pr "@;double %s;" varval;
        write_decoupled_expr ~env dgfip_flags oc vardef varval body;
        write_decoupled_expr ~env dgfip_flags oc res_def res_val followup;
        pr "@;}@]"

let write_c_expr ~env dgfip_flags oc res_def res_val expr =
  expr |> build_expression
  |> write_decoupled_expr ~env dgfip_flags oc res_def res_val
