...
(* let rec build_adv_transitive_composition ?safe_def = function *)
(*   | AtomicExpr e -> AtomicExpr (build_transitive_composition ?safe_def e) *)
(*   | Cond (c, t, e) -> *)
(*       Cond *)
(*         ( build_adv_transitive_composition ?safe_def c, *)
(*           build_adv_transitive_composition ?safe_def t, *)
(*           build_adv_transitive_composition ?safe_def e ) *)
(*   | Let { vardef; varval; body; followup } -> *)
(*       Let *)
(*         { *)
(*           vardef; *)
(*           varval; *)
(*           body = build_adv_transitive_composition ?safe_def body; *)
(*           followup = build_adv_transitive_composition ?safe_def followup; *)
(*         } *)
  ...
  let supzero se =
    make_let se (fun vardef varval ->
        ...
        let value_comp =
          ...
          Constr.Ite (cond, varval, Lit 0.0)
        in
        atomic
        @@ build_transitive_composition ~safe_def:false
             { set_vars = []; def_test; value_comp })
