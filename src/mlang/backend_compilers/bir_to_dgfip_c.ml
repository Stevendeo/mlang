...
let pop_scope ~id env =
  match env.scopes with
  | [] -> Format.ksprintf failwith "Pushing non existent scope %s" id
  | Id i :: tl ->
      if id = i then env.scopes <- tl
      else
        Format.ksprintf failwith "Pushing scope %s while top scope is %s" id i
  | Sanitize { san_id; _ } :: _ ->
      Format.ksprintf failwith
        "Pushing scope %s, but top of stack is Sanitizer %i" id san_id

let pop_sanitizer ~id env =
  match env.scopes with
  | [] -> Format.ksprintf failwith "Pushing non existent Sanitizr %i" id
  | Id i :: _ ->
      Format.ksprintf failwith "Pushing sanitizer %i met scope %s" id i
  | Sanitize { san_id; san } :: tl ->
      if id = san_id then begin
        san ();
        env.scopes <- tl
      end
      else
        Format.ksprintf failwith "Pushing sanitizer %i while expected %i" id
          san_id
...
let rec generate_stmt (env : env) (dgfip_flags : Dgfip_options.flags)
  ...
  | Iterate (var, al, var_params, stmts) ->
      ...
      let scope = fresh_scope ~var env in
      (* !!! *)
      pr "@;@[<v 2>{";
  ...
  | ArrangeEvents (sort, filter, add, stmts) ->
      ...
      in
      let id = add_sanitizer ~f:pp_sanitize env in
      pr "@;@[<v 2>{";
