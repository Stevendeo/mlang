...
let rec generate_stmt (env : env) (dgfip_flags : Dgfip_options.flags)
  ...
  | Iterate_values (var, var_intervals, stmts) ->
      ...
      (* !!! *)
      let itval_val = Env.gen_val ~env None var in
      (* !!! *)
  ...
  | ArrangeEvents (sort, filter, add, stmts) ->
      ...
      | Some (var, expr) ->
          ...
          let ref_def = Env.gen_def ~env None var in
          (* !!! *)
          let ref_val = Env.gen_val ~env None var in
          (* !!! *)
          let cond = D.fresh_c_local "cond" in
          let cond_def = cond ^ "_def" in
          let cond_val = cond ^ "_val" in
      ...
      | Some (var0, var1, expr) ->
          ...
          pr "@;@[<v 2>{";
          (* Comparaison *)
          let ref0_def = Env.gen_def ~env None var0 in
          (* !!! *)
          let ref0_val = Env.gen_val ~env None var0 in
          (* !!! *)
          let ref1_def = Env.gen_def ~env None var1 in
          (* !!! *)
          let ref1_val = Env.gen_val ~env None var1 in
          (* !!! *)
  ...
  | Restore (al, var_params, evts, evtfs, stmts) ->
      ...
      List.iter
        (fun m_a ->
          match Pos.unmark m_a with
          | Com.VarAccess (m_sp_opt, var) ->
              let def_ptr = Env.gen_def_ptr ~env m_sp_opt var in
              let val_ptr = Env.gen_val_ptr ~env m_sp_opt var in
        ...
        (fun (var, vcs, expr, m_sp_opt) ->
          ...
          Com.CatVar.Map.iter
            (fun vc _ ->
              ...
              pr "@;char %s;@;double %s;" cond_def cond_val;
              pr "@;%s = (T_varinfo *)tab_%s;" ref_info it_name;
              (* TODO: replace var_spaces access in case None *)
  ...
  | CleanErrors -> pr "@;nettoie_erreur(irdata);"
  | CleanFinalizedErrors -> pr "@;nettoie_erreurs_finalisees(irdata);"
...
let generate_function_prototype (add_semicolon : bool) (oc : Format.formatter)
  ...
  let pp_args fmt args =
    List.iteri
      ...
      args
  in
  pr "int %s(T_irdata* irdata, char *res_def, double *res_val%a)%s" fn pp_args
    fd.target_args
...
let generate_functions (dgfip_flags : Dgfip_options.flags) (p : Mir.program)
  ...
  List.iter
    (fun (name, ({ target_file; _ } : Mir.target)) ->
      ...
      generate_function dgfip_flags p fmt name)
    functions
...
let print_local_variables env oc =
  let pr m = Format.fprintf oc m in
  if env.uses_saisie_def then pr "@;char *def_saisie = irdata->def_saisie;";
  if env.uses_saisie_val then pr "@;double *saisie = irdata->saisie;";
  if env.uses_calculee_def then
    pr "@;char *def_calculee = irdata->def_calculee;";
  if env.uses_calculee_val then pr "@;double *calculee = irdata->calculee;";
...
(* We generate the target in five steps:
   ...
   - then, the body of the function that uses some variables we did not define
   yet (this part is printed in a temporary buffer);
   - given the variables used in the functions's body, print the proper variables
   declaration;
   - print the buffer's contents;
   - print the rest.
   This allows to generate code that do not define variables that will not be used.
 *)
let generate_target (dgfip_flags : Dgfip_options.flags) (p : Mir.program)
  ...
  let final_pr m = Format.fprintf final_oc m in
  ...
  if tf.target_sz_tmps > 0 then
    final_pr "@;irdata->tmps_org = irdata->tmps_org - %d;" tf.target_sz_tmps;
  final_pr "@;irdata->nb_refs_target = %s;" sav_nb_refs;
  final_pr "@;irdata->nb_tmps_target = %s;" sav_nb_tmps;
  final_pr "@;return irdata->discords;";
  final_pr "@]@;}@."
(* Given the state of *)
